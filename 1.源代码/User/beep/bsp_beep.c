...
{	
	...
	RCC_APB2PeriphClockCmd(Execute3_GPIO_CLK ,ENABLE);		
	/* 相关端口引脚初始化 */
	GPIO_InitStructure.GPIO_Pin = Execute3_GPIO_PIN;	
	GPIO_Init(Execute3_GPIO_PORT, &GPIO_InitStructure);
	
	/* 默认输出高电平 */
	GPIO_SetBits(Execute3_GPIO_PORT, Execute3_GPIO_PIN);
	Execute3_OFF;
}
